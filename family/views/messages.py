...
def messages():
    ...
    messages = Message.query(
        Message.posted_date > cutoff_time, ancestor=ancestor_key
    ).order(-Message.posted_date)
...
def message_new():
    ...
    if form.validate():
        ...
        if photo_url:
            ...
            image = "https://%s/static/images/male_bust.jpg" % current_app.settings.get(
                "host.name"
            )
        html_body = render_template(
            "email/new_message_posted.html",
            **dict(
                author=author,
            ...
            if subscriber.primary_email:
                ...
                mail.send_mail(
                    "{email_from} <{email_address}>".format(
                        ...
                        email_address=current_app.settings.get(
                            "messageboard.email.address"
                        ),
                    ),
